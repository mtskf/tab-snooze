...
declare const global: typeof globalThis & { chrome: typeof chrome };
...
vi.mock('../utils/timeUtils', () => ({
  ...
  parseTimeString: vi.fn((timeStr: string | undefined) => {
    if (!timeStr) return 8;
...
describe('Popup', () => {
  beforeEach(() => {
    ...
    if (!global.chrome.tabs) global.chrome.tabs = {} as typeof chrome.tabs;
    if (!global.chrome.runtime) global.chrome.runtime = {} as typeof chrome.runtime;

    global.chrome.runtime.openOptionsPage = vi.fn();
    global.chrome.runtime.getURL = vi.fn((path: string) => `chrome-extension://test/${path}`);

    // Mock tabs.query to support callback
    (global.chrome.tabs.query as ReturnType<typeof vi.fn>).mockImplementation((query: chrome.tabs.QueryInfo, callback?: (tabs: chrome.tabs.Tab[]) => void) => {
        const tabs = [{ id: 1, url: 'https://example.com', title: 'Test Tab' }] as chrome.tabs.Tab[];
        if (callback) callback(tabs);
        return Promise.resolve(tabs);
    });

    // Mock runtime.sendMessage to support callback
    (global.chrome.runtime.sendMessage as ReturnType<typeof vi.fn>).mockImplementation((msg: { action: string }, callback?: (response: unknown) => void) => {
